...
class AttentionLSTMCell(LSTMCell):
    ...
    @property
    def features(self):
        tanh_layer1 = tf.tanh(tf.matmul(self.w_p, self.r[-1]) + tf.matmul(self.w_x, self.h[-1]))
        tanh_layer2 = tf.tanh(tanh_layer1)
        return tf.tanh(tanh_layer2)
...
class RNN(TensorFlowTrainable):
    ...
    def get_predictions(self):
        biases = self.get_biases(dim_out=2, name="biases")
        weights = self.get_weights(dim_in=self._num_units, dim_out=2, name="weights")
    ...
    def loss(self, targets):
        if self.hiddens is None:
            ...
            return loss, loss_summary, accuracy, accuracy_summary, predictions
