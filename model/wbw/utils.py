...
def load_data(data_dir="../dataset/", word2vec_path="w2v.bin",test=False):
    ...
    if not test:
       ...
       for type_set in ["train", "test"]: 
           ...
           data_ids[type_set] = {"ID": df[["ID"]].values}
       tokenized_dataset = simple_preprocess(dataset=dataset, word2vec=word2vec)
       print "dataset: done\n"
       return word2vec, tokenized_dataset, data_ids
    else:
       print "\nLoading word2vec:"
       word2vec = {}
       word2vec = gensim.models.KeyedVectors.load_word2vec_format(word2vec_path, binary=True)
       ...
       print "\nLoading testdata:"
       num = input("num:")
       for type_set in ["test"]: 
           df = pd.read_csv(os.path.join(data_dir,"test{}.csv".format(num)), delimiter="	")
           df = df.sample(frac=1).reset_index(drop=True)
           dataset[type_set] = {"headline": df[["Headline"]].values, "body": df[["articleBody"]].values, "targets": df[["Stance"]].values}
           data_ids[type_set] = {"ID": df[["ID"]].values}
       tokenized_dataset = simple_preprocess(dataset=dataset, word2vec=word2vec)
...
def simple_preprocess(dataset, word2vec):
    ...
    for type_set in dataset:
        ...
        #map_targets = {"True": 0, "False": 1, "Can not judge": 2, "Unrelated": 3}
        map_targets = {"True": 0, "False": 1}
        #map_targets = {"entailment": 0, "contradiction": 1,"neutral": 2, "unrelated": 3}
...
def train(word2vec, dataset, data_ids, parameters, class_weights):
    print "s"
    ...
    with tf.device(device_string):
        ...
        loss, loss_summary, accuracy, accuracy_summary, predictions = body.loss(targets=targets_ph)
        ...
        tf.train.write_graph(sess.graph_def, modeldir, "graph.txt", as_text=True)
        ...
        train_batches = batcher.batch_generator(dataset=dataset["train"], data_ids=data_ids["train"], num_epochs=parameters["num_epochs"], batch_size=parameters["batch_size"]["train"], sequence_length=parameters["sequence_length"])
        ...
        for train_step, (train_batch, epoch) in enumerate(train_batches):
            ...
            if train_step % 10 == 0:
                ...
                sys.stdout.flush()
            if train_step % 100 == 0:
                test_batches = batcher.batch_generator(dataset=dataset["test"], data_ids=data_ids["test"], num_epochs=1, batch_size=parameters["batch_size"]["test"], sequence_length=parameters["sequence_length"],return_body=True)
                for test_step, (test_batch,_) in enumerate(test_batches):
                    ...
                    summary_str, test_loss, test_accuracy, test_predictions = sess.run([test_summary_op, loss, accuracy, predictions], feed_dict=feed_dict)
                    print"\nTEST | loss={0:.2f}, accuracy={1:.2f}%   ".format(test_loss, 100. * test_accuracy)
                    predictions_name = []
                    test_predictions = map(str,test_predictions)
			...
			if test_predictions[predictions_num] == "1":
                           ...
                           #predictions_name.append(["Can not judge"])
